...
## Ownership Boundary
This folder owns presentation, section ordering, route-level guidance, and pre-test page flow only. It must not own server freshness decisions, completion persistence, unlock writes, or analytics aggregation. Those belong to the learning backend and the progress / assessment API contract.
