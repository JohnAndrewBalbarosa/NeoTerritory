...
It should also keep a short local retention window for recent runs. The default review store should preserve the latest 150 runs locally, and the PM should be able to download a spreadsheet export (`.xlsx`) that includes response rows, log rows, and raw performance measurements such as elapsed time and space usage when those metrics are captured.
  ...
  "exports": [
    {
      ...
      "sheets": ["responses", "logs", "performance", "raw"]
    }
  ]
}
```
...
- The PM can download an Excel spreadsheet with response rows, logs, and raw performance data.
