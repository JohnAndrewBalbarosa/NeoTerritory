...
This document shapes how implementation work should be carried out across the NeoTerritory repository. In practice it sits ahead of code changes: an agent or contributor reads these instructions first, then applies them while editing the C++ parser, the supporting backend, or the surrounding docs so the resulting implementation stays aligned with the project's architecture and coding conventions.

## Activity Diagram
```mermaid
flowchart TD
    Start([Start])
    N0[Read the repository-wide implementation instructions]
    N1[Inspect the target module or file]
    N2[Apply changes with the documented constraints]
    N3[Validate the result against the project conventions]
    N4[Update code or docs consistently]
    End([End])
    Start --> N0
    N0 --> N1
    N1 --> N2
