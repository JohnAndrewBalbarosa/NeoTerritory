...
The flow is intentionally split into smaller slices so the major intent of parse_tree_dependency_utils.hpp stays readable. Each slice names the stage it is covering, gives a quick summary, and explains why that stage is separated from the next one.
...
Names worth recognizing while reading: DependencySymbolNode, collect_dependency_class_nodes, and collect_dependency_function_nodes.

It leans on nearby contracts or tools such as parse_tree.hpp, cstddef, string, and vector.
