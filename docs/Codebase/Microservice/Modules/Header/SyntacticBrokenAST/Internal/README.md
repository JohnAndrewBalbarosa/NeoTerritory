# Internal

- Folder: docs/Codebase/Microservice/Modules/Header/SyntacticBrokenAST/Internal
- Descendant source docs: 3
- Generated on: 2026-04-22

## Logic Summary
Internal header contracts supporting the syntactic subsystem.

## Documents By Logic
### Internal Contracts
- parse_tree_hash_links_internal.hpp.md : Declares the public interfaces and shared data types for the generic parse and analysis pipeline.
- parse_tree_internal.hpp.md : Declares the public interfaces and shared data types for the generic parse and analysis pipeline.
- parse_tree_symbols_internal.hpp.md : Declares the public interfaces and shared data types for the generic parse and analysis pipeline.

## Reading Hint
- This folder is mostly leaf-level. Read the local file docs to understand the logic in this area.

