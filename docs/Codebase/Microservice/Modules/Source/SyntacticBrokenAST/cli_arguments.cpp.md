...
flowchart TD
    Start([Start])
    N0[Execute split whitespace tokens to assemble tree or artifact structures and iterate over the active collection]
    N1[Execute format usage with argc]
    N2[Execute parse cli arguments to branch on runtime conditions]
    N3[Execute if]
    End([End])
    Start --> N0
    N0 --> N1
