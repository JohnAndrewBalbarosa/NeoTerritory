...
Quick summary: This slice shows the opening intent of build_source_type_skeleton_lines.cpp and the first major actions that frame the rest of the flow.
Why this is separate: build_source_type_skeleton_lines.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
...
Quick summary: This slice covers the first branch-heavy continuation of build_source_type_skeleton_lines.cpp after the opening path has been established.
Why this is separate: build_source_type_skeleton_lines.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
    N0["Branch condition"]
