...
Quick summary: This slice shows the opening intent of build_target_type_skeleton_lines.cpp and the first major actions that frame the rest of the flow.
...
flowchart TD
    ...
    N1["Enter build_target_type_skeleton_lines()"]
    N2["Build output"]
    N3["Read lines"]
    N4["More items?"]
    N5["Record output"]
...
Quick summary: This slice covers the first branch-heavy continuation of build_target_type_skeleton_lines.cpp after the opening path has been established.
Why this is separate: build_target_type_skeleton_lines.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
    N0["Branch condition"]
