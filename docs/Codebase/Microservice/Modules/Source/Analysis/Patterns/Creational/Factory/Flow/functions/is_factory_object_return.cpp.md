...
Quick summary: This slice shows the opening intent of is_factory_object_return.cpp and the first major actions that frame the rest of the flow.
...
Quick summary: This slice covers the first branch-heavy continuation of is_factory_object_return.cpp after the opening path has been established.
Why this is separate: is_factory_object_return.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
    N0["Branch condition"]
