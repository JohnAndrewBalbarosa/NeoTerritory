...
- Folder: docs/Codebase/Microservice/Modules/Source/Creational/Factory
...
Quick summary: This slice shows the opening intent of Factory and the first major actions that frame the rest of the flow.
...
Quick summary: This slice covers the first branch-heavy continuation of Factory after the opening path has been established.
Why this is separate: Factory has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
...
- factory_pattern_logic.cpp.md : Implements creational pattern detection over the generic parse tree.
