...
Quick summary: This slice shows the opening intent of cli_arguments.cpp and the first major actions that frame the rest of the flow.
...
Quick summary: This slice covers the first branch-heavy continuation of cli_arguments.cpp after the opening path has been established.
Why this is separate: cli_arguments.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
    N0["Normalize text"]
...
Quick summary: This slice captures the mid-flow handoff in cli_arguments.cpp where preparation turns into deeper processing.
Why this is separate: cli_arguments.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
    N0["Return result"]
