...
**One round — strict AND of category combos.** Each pattern JSON declares `signature_categories: [name, ...]` — a flat array, no weights, no ranks. The connotation dictionary at `pattern_catalog/lexeme_categories.json` stores each category as a list of *combos*. A combo is an ordered sequence of consecutive token lexemes that together carry the category's pattern meaning. A category is satisfied for a class when at least one of its combos matches a window of consecutive tokens in the class. A pattern survives only when every category in its `signature_categories` is satisfied. Empty `signature_categories` = pattern not yet opted into the connotation rule; it passes through on `ordered_checks` alone.
