...
## D82 — Sample-side tag manifest seeds deterministic ambiguity resolution in the e2e (2026-05-25)

**Problem.** The studio e2e (`Codebase/Frontend/playwright/tests/all-samples.spec.ts`) kept tripping on "ambiguous tags" even though every fixture sample's true GoF pattern is already known (it lives under a pattern-named directory — `singleton/`, `builder/`, `adapter/`, …). Co-emit ambiguity is **by design** (D21/D38/D80): the microservice deliberately emits competing patterns on the same class (Adapter+Decorator on wrap+forward, Builder+MethodChaining on fluent `*this` chains, `strategy_interface` on every behavioural sample) and the backend AI / user disambiguates. So the friction is **not** an algorithm bug to "fix" — it is a missing piece of *test* knowledge: the e2e did not feed the sample's known pattern back into resolution. Its `resolveAllAmbiguousClasses` helper clicked the **arbitrary first** candidate chip, which is order-dependent and flaky, and the run-tests gate (`409`/`400 AMBIGUOUS_TAGS`) recurred.

**Decision (CI e2e scope only — backend `candidateFilter`/`analysis.ts` and the live-studio disambiguation UX are untouched; co-emit ambiguity stays by-design for real users):**

- New sample-side tag manifest `Codebase/Microservice/samples/sample-tags.json` is the **single source of truth** for each test sample's canonical tag(s). One row per sample: `file` (dir/filename relative to `samples/`), `family`, `kind` (`positive`/`negative`), optional `classHint`, and `tags` — a **priority-ordered** list of acceptable canonical pattern names (the exact labels chips/badges render: `Builder`, `Singleton`, `MethodChaining`, `Adapter`, `Decorator`, `Proxy`, `Pimpl`, `Strategy`, `Factory`, …). `kind: "negative"` rows carry `tags: []` by design.
- The manifest lives **with the samples** (the fixture fact "this sample IS a Builder"), decoupled from "what the matcher co-emitted" (algorithm behaviour). `samples/` is not under the `docs/Codebase` mirror, so no `.md` twin is required.
- `all-samples.spec.ts` loads the manifest and resolves ambiguity by clicking the chip whose label matches the sample's known tag **in priority order**; it only falls back to `.first()` (annotated) when no chip matches a known tag. Resolution is now deterministic and order-independent.
- **Drift guard (future-proofing):** the spec asserts at load that every `positive` sample it iterates has a manifest row with at least one tag, and that every manifest row maps to a real sample file. Add a sample → add a manifest row, or the spec fails fast with a clear message — the same "manifest is the gate" contract as `tests/routes.manifest.json`.

**Why not auto-resolve in the backend / whole studio:** that would move disambiguation out of the AI/user layer and contradict D38/D80 (ambiguity is information; the AI assigns the final role). The known-tag seed is a property of the *fixture*, not the engine, so it belongs in test infrastructure only.

## D83 — Adapter/wrapping e2e accepts Strategy as a documented limit; cascade-collapse root cause logged (2026-05-25)

**Symptom.** CI `all-samples.spec.ts` failed fast (~2s, before the GDB pipeline) on `adapter/payment_gateway_adapter.cpp` (expected `/adapter/i`) and `wrapping/logging_proxy.cpp` (expected `/adapter|proxy|decorator|wrap/i`). Not a flake, not a harness/edit regression.

**Root cause (verified).** Raw microservice detection is CORRECT — a fresh CI-equivalent build tags `LegacyPaymentAdapter` and `LoggingDataService` as **Adapter + Decorator** (plus StrategyConcrete). The loss happens in the **frontend** `deriveAnnotatedModel` subclass cascade (`Codebase/Frontend/src/logic/annotatedModel.ts`, case b/e, ~line 532): the adapter implements its target interface (`IPaymentProcessor` / `IDataService`), which the structural matcher also tags **Strategy** (an interface with a polymorphic op looks like a Strategy participant). Strategy is a *propagating* pattern, so the cascade treats the adapter as a propagated Strategy subclass and **collapses the child's candidates to `[Strategy]`**, dropping the independently-detected Adapter/Decorator chips. The studio (and the e2e badge/candidate read) therefore never surface Adapter/Proxy/Decorator for these classes.
