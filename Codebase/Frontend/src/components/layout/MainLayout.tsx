  ...
  return (
    <div className="shell ide-shell">
      <header className="topbar">
        <div className="brand">
          <p className="eyebrow">DEVCON / NEOTERRITORY</p>
          <h1><span className="brand-prompt">&gt;_</span> Studio</h1>
          <p className="lede">
            Upload or paste C++ source to detect design patterns, generate annotations, and validate structure.
          </p>
        </div>
      ...
      <div className="ide-statusstrip" role="status" aria-live="polite">
        <div className={`ide-status-item`}>
          <span className={`ide-status-dot ide-status-dot--${status.kind === 'ok' ? 'ok' : status.kind === 'error' ? 'err' : 'warn'}`} />
          {status.title}
        </div>
        {status.detail && (
          <div className="ide-status-item">{status.detail}</div>
        )}
        <div className={`ide-status-item`}>
          <span className={`ide-status-dot ${msState === 'online' ? 'ide-status-dot--ok' : msState === 'offline' ? 'ide-status-dot--err' : 'ide-status-dot--warn'}`} />
          ms: {msLabel}
        </div>
        <div className="ide-statusstrip-spacer" />
        <div className="ide-status-item ide-status-item--right">{user?.username ?? ''}</div>
      </div>
