...
export default function MechanicsPage() {
  ...
  return (
    <main className="nt-mech" id="main">
      ...
      {/* Trophy-vs-data-structure section (D73). Explains why our parse
          tree + virtual structural copy maps naturally onto the Testing
          Trophy's layers, plus the honest limits of a tree-only approach. */}
      <section className="nt-mech__trophy" aria-labelledby="mech-trophy">
        <h2 id="mech-trophy" className="nt-mech__trophy-title">
          How our data structure helps Trophy testing
        </h2>
        <p className="nt-mech__trophy-lede">
          The virtual structural copy of the parse tree (stage 2) is the single artefact every
          test layer in our Testing Trophy reads from. That shared substrate is what lets each
          layer cost almost nothing once detection is done.
        </p>

        <div className="nt-mech__trophy-grid">
          <div className="nt-mech__trophy-col nt-mech__trophy-col--pros">
            <h3>Pros</h3>
            <ul>
              <li>
                <strong>Cheap unit tests.</strong> Detection already pinned the load-bearing
                methods on each class node. The Unit-test layer just walks those nodes and emits
                a scaffold per function — no extra parsing.
              </li>
              <li>
                <strong>Integration tests share the tree.</strong> The virtual copy is the same
                object the backend and AI layer consume. Tests that exercise the seams between
                microservice, backend, and AI all read from one tree shape, so a single tree
                fixture covers many cross-process scenarios.
              </li>
              <li>
                <strong>E2E tests anchor to class names.</strong> Playwright assertions like
                &ldquo;a class named X with pattern Y appears on the Patterns tab&rdquo; flow
                directly from tree node labels - no extra reflection needed.
              </li>
              <li>
                <strong>Static analysis benefits indirectly.</strong> Lexical categories
                attached at stage 1 give clang-tidy and ESLint contextual hints (which tokens
                are stdlib symbols vs project identifiers) when those tools run on the same
                files.
              </li>
              <li>
                <strong>Deterministic substrate.</strong> The tree is rebuilt the same way
                every run, so test fixtures stay stable across versions of the AI provider or
                backend.
              </li>
            </ul>
          </div>

          <div className="nt-mech__trophy-col nt-mech__trophy-col--cons">
            <h3>Cons</h3>
            <ul>
              <li>
                <strong>Tree-shaped tests only.</strong> Our scaffolds reason about classes,
                methods, and their immediate relationships. Behaviours that span multiple
                translation units or runtime threads are out of scope.
              </li>
              <li>
                <strong>No data-flow tests.</strong> The tree does not track values across
                expressions; we cannot generate a test that asserts &ldquo;variable X equals 42
                after this loop&rdquo; from the structural facts alone.
              </li>
              <li>
                <strong>No timing or perf tests.</strong> Stage durations are captured but the
                tree itself does not encode latency expectations; perf tests must be hand-written.
              </li>
              <li>
                <strong>AI-doc layer adds non-determinism.</strong> The structural detection is
                deterministic, but the AI documentation is not. Integration tests stub the AI
                client to keep the seam test deterministic; the AI itself is exercised
                end-to-end only when explicitly opted in.
              </li>
            </ul>
          </div>

          <div className="nt-mech__trophy-col nt-mech__trophy-col--limits">
            <h3>Limits (tree-structure related)</h3>
            <ul>
              <li>
                <strong>Class-level only.</strong> The matcher operates on complete class or
                struct declarations. Free functions outside a class are not tracked.
              </li>
              <li>
                <strong>Single-file scope per analysis.</strong> Cross-file relationships are
                inferred only from name references in the function bodies the analyser
                actually receives.
              </li>
              <li>
                <strong>No template instantiation.</strong> Templates appear as their generic
                form; per-instantiation tests would require running the compiler, not the
                parser.
              </li>
              <li>
                <strong>No semantic understanding.</strong> The tree encodes structure, not
                meaning. A pattern can be structurally present but semantically wrong; the AI
                doc layer catches this when configured, but no structural test can.
              </li>
              <li>
                <strong>Catalog-bounded detection.</strong> Patterns outside the JSON catalog
                are invisible. Adding new ones is a catalog edit (no recompile), but the matcher
        ...
        <p className="nt-mech__trophy-foot">
          <strong>Honest bottom line:</strong> NeoTerritory&rsquo;s data structure makes Trophy
          testing cheap where structure matters and silent where structure does not. Tests that
          need data flow, timing, or cross-translation-unit behaviour need a different tool.
        </p>
      </section>
