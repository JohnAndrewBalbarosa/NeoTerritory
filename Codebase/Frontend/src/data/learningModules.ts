...
const PATTERN_THEORY: Record<string, ReadonlyArray<ExamQuestion>> = {
  ...
  factory: [
    ...
    },
    {
      type: 'identification',
      taxonomy: 'understanding',
      question: 'By hiding the `new` keyword behind a method in a base class, what responsibility are you deferring to the subclasses?',
      scenario: 'You are writing a cross-platform GUI. Instead of using `new WindowsButton()` directly in your business logic, you call `guiFactory->createButton()`. The exact class of the button is decided by whichever specific factory subclass (WindowsFactory or MacFactory) is currently in use.',
      expectedTokens: ['instantiation', 'creation'],
      explanation: 'Factory Method defers the *instantiation* (creation) of concrete objects to subclasses.',
    },
    {
      type: 'studio',
      taxonomy: 'creating',
      prompt: 'Implement a "Logistics" base class with a pure virtual "createTransport()" method (the Factory Method). Then create a "RoadLogistics" subclass that overrides it to return a "Truck" object. This defers instantiation to the subclass.',
      targetPatternSlug: 'factory',
      starterCode: 'class Transport { public: virtual ~Transport()=default; };\nclass Truck : public Transport {};\n\nclass Logistics {\npublic:\n  // TODO: declare pure virtual createTransport()\n};\n\nclass RoadLogistics : public Logistics {\npublic:\n  // TODO: override createTransport to return a Truck\n};',
      explanation: 'The student must define a creator base class and defer object creation to a concrete subclass.',
    },
  ],
  'abstract-factory': [
  ...
  'template-method': [
    ...
    },
    {
      type: 'identification',
      taxonomy: 'understanding',
      question: 'What do we call the blank or overridable steps (like `parseData()`) that the base class calls, which subclasses must fill in?',
      scenario: 'You are writing a data processing pipeline. The overarching algorithm is always: `readData()` -> `parseData()` -> `save()`. You put this exact sequence in a non-virtual base class method. However, you leave `parseData()` as a pure virtual function so subclasses can provide their own specific logic.',
      expectedTokens: ['hook', 'hooks', 'primitive', 'primitive steps'],
      explanation: 'These overridable steps are called primitive operations or "hooks". The base class provides the template, and subclasses provide the hooks.',
    },
    {
      type: 'studio',
      taxonomy: 'creating',
      prompt: 'Implement a "DataParser" base class. Write a non-virtual "parse()" method (the template method) that calls "open()", "extract()", and "close()". Make "extract()" a pure virtual method (`= 0`) so subclasses are forced to provide it.',
      targetPatternSlug: 'template-method',
      starterCode: 'class DataParser {\npublic:\n  void parse() {\n    // TODO: call open, extract, close\n  }\n  virtual void open() {}\n  virtual void close() {}\n  // TODO: declare pure virtual extract()\n};',
      explanation: 'The student must define the skeleton algorithm in a method that calls a pure virtual hook.',
    },
  ],
