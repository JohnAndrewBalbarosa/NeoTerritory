// Interpreter — flexible test driver.
...
static int run_tests() {
    ...
    nt::emit_criterion("behavioural.interpreter", "{{CLASS_NAME}}", "pass",
        "Interpreter type compiles and is visible to consumers.");

    if constexpr (std::is_abstract_v<T>) {
        nt::emit_criterion("behavioural.interpreter", "{{CLASS_NAME}}", "pass",
            "Type is abstract — its virtual surface enforces dispatch through a concrete subtype.");
    } else {
        nt::emit_criterion("behavioural.interpreter", "{{CLASS_NAME}}", "skip",
